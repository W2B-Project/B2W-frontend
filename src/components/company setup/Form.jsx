...
import { AddUserPic, getCompanyData, setupNewCompany } from "../../Api_Calls/SetupServices"
function Form({ getnextstep, btn_text }) {
    const [link, setlink] = useState('')
    const [Social, setSocial] = useState([])
    const { companyInfo, setCompanyInfo, setComData,setPic,Pic } = useContext(SetupContext)
    ...
    return (
        <form>
            {/* image upload */}
            <input type="file" id="inputt" className="hidden" onChange={handleImageUpload} />
            <label htmlFor="inputt">
                <img src={Pic?Pic:setup.defImg} loading="lazy" alt="upload image" className="m-auto rounded-full h-28 w-28" />
            </label>
            {/* user info */}
            <div className="mt-5">
