...
const services = [
    ...
    { link: "chatbot", image: secondImage, name: "Chatbot Interview" },
    { link: "Signify", image: thirdImage, name: "Signify" }
];

function ServicesMenu({setService,service}) {
    return (
        <div className="shadow p-5 rounded-xl bg-white">
            <h3 className="text-xl font-semibold mb-3">Services</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {services.map(({ link, image, name }) => (
                    <ServiceItem key={link} serviceImage={image} serviceName={name} setService={setService} service={service} />
                ))}
            </div>
        </div>
    );
